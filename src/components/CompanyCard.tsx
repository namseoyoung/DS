...
const companyProfiles: Record<CompanyId, CompanyProfile> = {
  sanghyun: {
    ...
    greeting:
      "상현회사는 흔들리지 않는 운영력과 빠른 실행으로 시장 기회를 붙잡습니다. 참가자의 투자가 더 큰 성장으로 이어지도록 만들겠습니다.",
  },
  seoyoung: {
    initials: "SY",
    tagline: "세밀한 전략과 균형 잡힌 포트폴리오의 회사",
    greeting:
      "서영회사는 데이터를 감각적으로 읽고 안정적인 판단으로 성과를 만듭니다. 신뢰받는 결과를 보여드리겠습니다.",
  },
  ain: {
    initials: "AI",
    tagline: "새로운 아이디어를 빠르게 실험하는 혁신 기업",
    greeting:
      "아인회사는 도전적인 기술과 창의적인 사업 모델로 성장합니다. 과감한 선택이 더 큰 가능성으로 이어지게 하겠습니다.",
  },
  donghyun: {
    initials: "DH",
    tagline: "현장 감각과 추진력이 강한 성장 기업",
    greeting:
      "동현회사는 빠른 판단과 강한 추진력으로 흐름을 바꿉니다. 시장의 속도를 기회로 바꾸겠습니다.",
  },
  yeil: {
    initials: "YL",
    tagline: "작지만 집중력 있는 반전형 기업",
    greeting:
      "예일회사는 낮은 출발점에서도 집중력 있는 전략으로 반전을 노립니다. 끝까지 지켜볼 만한 회사를 만들겠습니다.",
  },
};

export function CompanyCard({ company, investedAmount, canInvest, onSelect }: CompanyCardProps) {
    ...
    <article className="w-full rounded-card border border-slate-200 bg-white p-6 text-left shadow-soft transition duration-300 active:scale-[0.99]">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <div className="rounded-button bg-slate-50 px-3 py-2">
          <span className="text-xs font-medium text-slate-500">내 투자금</span>
          <p className="mt-1 text-sm font-bold text-slate-950">{formatWon(investedAmount)}</p>
        </div>
        <button
          type="button"
          disabled={!canInvest}
...
function CompanyAvatar({ company, initials }: { company: Company; initials: string }) {
  return (
    <span
      className="grid h-12 w-12 shrink-0 place-items-center rounded-full text-sm font-black text-white shadow-soft"
...
function CompanyProfileModal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end bg-slate-950/40 px-4 pb-4 sm:items-center sm:justify-center sm:pb-0"
      onClick={onClose}
      role="presentation"
    >
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`${company.name} 소개`}
      >
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            <CompanyAvatar company={company} initials={profile.initials} />
            <div>
              <p className="text-xs font-semibold text-slate-400">기업 소개</p>
              <h3 className="mt-1 text-xl font-bold text-slate-950">{company.name}</h3>
            </div>
          </div>
          <button
            type="button"
